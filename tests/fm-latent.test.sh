...
if [ "${1:-} ${2:-}" = "pr view" ]; then
  case "$*" in
    # The poll asks for state on its own, then - only for a hibernating task -
    # for the head and review decision together. Answer each shape as gh would,
    # so a query shape change cannot pass here while failing against real gh.
    *'--json headRefOid,reviewDecision'*) printf '%s\t%s\n' "${FM_FAKE_GH_HEAD:-}" "${FM_FAKE_GH_REVIEW:-}" ;;
