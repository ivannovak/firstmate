...
family_for_basename() {
  case "$1" in
    ...
    fm-tangle-guard.test.sh|fm-update.test.sh|fm-absorb-upstream.test.sh)
      printf '%s\n' session-bootstrap
      ;;
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-latent.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
    fm-pr-review-activity.test.sh|fm-review-diff.test.sh|\
...
portable_serial_weight_hints() {
  cat <<'EOF'
tests/fm-absorb-upstream.test.sh 4277
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      printf '%s\n' secondmate
      ;;
    bin/fm-pr-*|bin/fm-latent.sh|bin/fm-task-safety-lib.sh|bin/fm-merge-local.sh|bin/fm-teardown.sh|bin/fm-review-diff.sh|\
