...
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, `bin/fm-bearings-snapshot.sh` provides the bounded bearings projection, and the opt-in captain board (`bin/fm-fleet-board.sh`, [configuration.md](configuration.md#captain-fleet-board-configfleet-board)) projects the same JSON into its four columns, so every view consumes one structured contract instead of reparsing raw fleet files and none of them stores fleet state of its own.
...
The one surface that survives every such drop is the readable home's captain holds, which are backlog facts rather than child-derived ones, so an unreconcilable home cannot hide what awaits the captain; [decision-hold-lifecycle.md](decision-hold-lifecycle.md#structured-read-surfaces) owns that carve-out and how each reader places those holds.
...
`/updatefirstmate` fast-forwards the running firstmate repo and registered secondmate homes from this fleet's configured update source, then re-reads updated instructions and nudges updated secondmates without touching project clones.
That source may be a fork rather than the open-source project, so a change is usable as soon as it lands there; [`docs/configuration.md`](configuration.md) owns the configuration and `bin/fm-absorb-upstream.sh` owns the separate, equally fast-forward-only path that takes upstream's work back in.
For a remote route, the configured code root updates from that same source on that host before the persistent home fast-forwards to the code-root commit.
