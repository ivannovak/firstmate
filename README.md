...
- **Optional fleet board** - opt in with one local `config/fleet-board` file for a read-only board of what needs you, what waits on someone else, what is moving, and what landed, grouped by thread and derived entirely from the backlog; it rebuilds when fleet state actually changes rather than on a timer, states its own age loudly enough that a stale board cannot be mistaken for a current one, and, when you explicitly serve it, publishes over Tailscale to your tailnet only, never through funnel.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
  ...
  treehouse worktree, Orca worktree, or isolated secondmate home
     │
     ├─ ship: project mode ► PR wait (eligible workers hibernate slot-free) ► merge ► teardown
     │
...
An eligible worker waiting only on an open GitHub PR can hibernate without completing the task: Firstmate pins the exact PR commit locally, releases the worker and Treehouse slot, keeps monitoring the PR, and reconstructs a fresh worker if review requires changes.
...
| `/updatefirstmate` | Self-update the running firstmate and its secondmates to the latest from this fleet's configured update source with fast-forward-only pulls, then re-read instructions and nudge secondmates |
