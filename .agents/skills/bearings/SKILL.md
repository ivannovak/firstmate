...
3. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   For a richer review surface, optionally offer the visual fleet board via `bin/fm-fleet-board.sh` rather than hand-building a second fleet-state renderer with raw `lavish-axi`, but only after the required digest is ready.
